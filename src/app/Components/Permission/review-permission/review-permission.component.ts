...
@Component({
  selector: 'app-review-permission',
  templateUrl: './review-permission.component.html',
  styleUrls: ['./review-permission.component.css']
})
export class ReviewPermissionComponent implements OnInit, OnDestroy {
  ...
  refreshEntitiesTable() {
    this.repo.getData('permissions', { review: -1, includes: 'Employee' })
      .subscribe(data => {
        console.log(data);
  ...
  reviewPermission() {
    ...
    permission.ReviewDecision = this.decision ? this.decision : false;
    permission.reviewRejectReason = this.reason;
    debugger;
    this.repo.saveElement('permissions', permission.id, permission)
      .subscribe(data => {
        $('#tblItems').DataTable().destroy();
        ...
        this.toastr.success('Great!', 'permission has been reviewed.');
        //alert('updated');
      }, err => {
        $('#tblItems').DataTable().destroy();
        this.refreshEntitiesTable();
